...
parser.add_argument("--feature", type=str, default="select_features.pt", help="torch feature file name")
parser.add_argument("--label", type=str, default="select_labels.npy", help="numpy label file name")
parser.add_argument("--cuda", type=int, default=1, help="if use cuda accelarate")
#parser.add_argument("--gpu_id", type=int, default=0, help="gpu id")
parser.add_argument("--max_iter", type=int, default=1000, help="Maximum number of iterations for the optimization")
parser.add_argument("--save_fig_name", type=str, default='result.png', help="fig name")
...
cuda = opt.cuda
save_fig_name = opt.save_fig_name
#gpu_id = opt.gpu_id
...
def tsne(X, max_iter, no_dims=2, initial_dims=50, perplexity=30.0):
	"""
...
if __name__ == "__main__":
	...
	color_set = ['red', 'green', 'blue', 'yellow', 'black', 'white']	
	data_path = './data/'
	
	X = torch.load(data_path + feature)
	if cuda: X = X.to('cuda')
	print('X size:', X.size())
	labels = np.load(data_path + label)
	print('labels size:', labels.shape)
	
	#if str(type(labels[0])) != "<class 'int'>":
	category_set = np.unique(labels).tolist()
	print('category_set:', category_set)
	
	category_index_list = []
	for i in range(len(category_set)):
		category_index_list.append(np.where(labels==category_set[i]))

	# confirm that x file get same number point than label file
	# otherwise may cause error in scatter
	assert(X.size()[0]==labels.shape[0])
	...
	for category_index in category_index_list:
		category_index = category_index[0].tolist()
		print(category_index)
		Y_new = []
		label_new = []
		for i in category_index:
			Y_new.append(Y[i])
			label_new.append(color_set[category_set.index(labels[i])])
		Y_new_list.append(np.array(Y_new))
		label_new_list.append(np.array(label_new))
		
	plt.figure()
	for i in range(len(category_set)):
		plt.scatter(Y_new_list[i][:, 0], Y_new_list[i][:, 1], c=label_new_list[i], label=category_set[i])
